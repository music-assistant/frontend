...
<template>
  <div v-if="enabledPref" class="visualizer-menu" @pointerdown.stop @click.stop>
    ...
    <button
      type="button"
      class="visualizer-menu__row visualizer-menu__settings-link"
      @click="openSettings"
    >
      <SettingsIcon :size="20" class="visualizer-menu__icon" />
...
const openSettings = () => {
  // Leaving the fullscreen player; the route change closes the menu.
...
.visualizer-menu__settings-link {
  width: 100%;
  border-radius: 6px;
  padding: 4px 0;
  text-align: left;
...
.visualizer-menu__settings-link:hover {
  background: var(--accent);
}
</style>
