<template>
  <v-card outlined @click="emit('click', item)" @click.right.prevent="emit('menu', item)">
    <MediaItemThumb :item="item" :size="size" />
    <div v-if="showCheckboxes" style="
        position: absolute;
        top: 0;
        background-color: #82b1ff94;
        height: 50px;
      ">
      <v-checkbox :model-value="isSelected" @click.stop @update:model-value="
        (x: boolean) => {
          emit('select', item, x);
        }
      " />
    </div>
    <div v-if="HiResDetails" class="hiresicon" :style="
    ...
    ">
      <v-tooltip bottom>
        ...
        <template #activator="{ props }">
          ...
          <img :src="iconHiRes" height="35" v-bind="props" :style="
            $vuetify.theme.current.dark
              ? 'object-fit: contain;'
    ...
    <v-list-item two-line style="padding-left: 8px; padding-right: 8px">
      <v-list-item-text>
        <p class="font-weight-bold line-clamp-1" style="color: primary; margin-top: 8px; margin-bottom: 8px">
          {{ item.name }}
        </p>
      </v-list-item-text>
      <v-list-item-subtitle v-if="'artists' in item && item.artists" class="line-clamp-2" style="margin-bottom: 8px">{{
        getArtistsString(item.artists)
      }}</v-list-item-subtitle>
      <v-list-item-subtitle v-else-if="'owner' in item && item.owner" class="line-clamp-2" style="margin-bottom: 8px">{{
        item.owner
      }}</v-list-item-subtitle>
