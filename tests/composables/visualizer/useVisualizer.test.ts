/**
 * Tests for the visualizer enabled-state fallback: a session with no stored
 * preference (a cast dashboard viewer has none and cannot set any) follows
 * the plugin's show_on_dashboards setting, while an explicit preference
 * always wins. Dashboard viewers that cannot render also report their
 * (negative) capability, since nothing else on such a display would.
 */
