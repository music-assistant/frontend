    ...
    <Container>
      <v-list>
        <ListItem v-for="providerMapping in itemDetails?.provider_mappings" :key="providerMapping.provider_instance">
          ...
          <template #subtitle>
            <span v-if="itemDetails.media_type == MediaType.TRACK">{{ providerMapping.audio_format.content_type }} |
              {{ providerMapping.audio_format.sample_rate / 1000 }}kHz/{{ providerMapping.audio_format.bit_depth }}
              bits |
            </span>
            <a v-if="providerMapping.url && !providerMapping.provider_domain.startsWith('file')" style="opacity: 0.4"
          ...
          <template #append>
            <!-- hi res icon -->
            <v-img v-if="providerMapping.audio_format.bit_depth > 16" :src="iconHiRes" width="30"
              :class="$vuetify.theme.current.dark ? 'hiresicondark' : 'hiresicon'" style="margin-right: 15px" />
            <audio v-if="getBreakpointValue('bp1') && itemDetails.media_type == MediaType.TRACK" name="preview"
